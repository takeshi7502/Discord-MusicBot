...
const command = new SlashCommand()
  .setName("cmd")
  .setDescription("Hiển thị danh sách lệnh ẩn của hệ thống (Chỉ dành cho Admin)")
  .setAdminOnly(true)
  .setRun(async (client, interaction) => {
    ...
    const helpEmbed = new EmbedBuilder()
      ...
      .setAuthor({
        name: `Lệnh Admin của ${client.user.username}`,
    ...
    if (tempArray.length > 0) {
      ...
      helpEmbed.setDescription("Không có lệnh tác vụ ẩn nào.");
    }

    helpEmbed.addFields({
      name: "Credits",
      value:
        `Phiên Bản Discord Music Bot: v${
    ...
    const getButtons = (pageNo) => {
      return new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("cmd_but_2_app")
          .setEmoji("◀️")
          .setStyle(ButtonStyle.Primary)
          .setDisabled(pageNo == 0),
        new ButtonBuilder()
          .setCustomId("cmd_but_1_app")
          .setEmoji("▶️")
          .setStyle(ButtonStyle.Primary)
          .setDisabled(pageNo >= maxPages - 1),
        new ButtonBuilder()
          .setCustomId("cmd_but_close_app")
          .setEmoji("❌")
