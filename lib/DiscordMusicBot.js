...
class DiscordMusicBot extends Client {
  ...
  createController(guild, player) {
    ...
    const buttonRow = new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setStyle(player.repeatMode !== "off" ? ButtonStyle.Success : ButtonStyle.Danger).setCustomId(`controller:${guild}:Loop`).setEmoji(player.repeatMode === "track" ? "🔂" : "🔁")
    );
    rows.push(buttonRow);

    // Row 2: Select Menu hàng đợi (chỉ hiển thị khi có bài đang chờ)
