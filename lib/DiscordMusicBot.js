  ...
  build() {
    ...
    this.manager
      .on("trackError", (player, track, payload) => {
        ...
        if (channel) channel.send({ embeds: [errorEmbed] });
      })
      ...
      .on("queueEnd", async (player, track) => {
        const autoQueue = player.get("autoQueue");
        // autoQueue is handled by onEmptyQueue.autoPlayFunction above
        ...
        if (!autoQueue) {
          ...
          try {
            if (!player.playing && !twentyFourSeven) {
              ...
              client.warn(
                `Có điều không mong muốn đã xảy ra với Bot ${player.guildId}`
              );
            }
            // Delete now playing message
            deleteNowPlayingMessage(client, player);
