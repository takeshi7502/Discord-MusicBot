...
const resolveIcon = require("../util/resolveIcon");
...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    .on("trackStart", async (player, track) => {
      ...
      title = title.replace(/\]/g, "").replace(/\[/g, "");
      const { iconURL: resolvedIconURL, files: iconFiles } = resolveIcon(client.config.iconURL);
      ...
      if (!autoQueue || !tracksAdded) {
        ...
        const showQueueEndAndDisconnect = async () => {
          let queueEmbed = new EmbedBuilder().setColor(client.config.embedColor).setAuthor({
            name: t("queue.ended"),
            iconURL: resolveIcon(client.config.iconURL).iconURL
          }).setFooter({
            text: t("queue.ended")
          }).setTimestamp();
