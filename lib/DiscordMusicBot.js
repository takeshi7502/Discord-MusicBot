...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    this.manager.nodeManager.on("connect", node => {
      ...
      this.log(t("system.lavalinkConnected", {
        id: node.id,
        host: h,
        port: p
      }));
      ...
      const p = node.options?.port || "N/A";
      const s = node.options?.secure ? "True" : "False";
      this.warn(t("system.lavalinkDisconnected", {
        id: node.id,
        host: h,
        port: p
      }));
      ...
      // Kiểm tra có node nào còn sống không
      ...
      this.warn(t("system.lavalinkError", {
        id: node.id,
        error: err.message
      }));
      // Chỉ báo 1 lần, đến khi connect lại mới báo tiếp
  ...
  createController(guild, player) {
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setStyle(ButtonStyle.Danger).setCustomId(`controller:${guild}:Stop`).setEmoji("⏹️"), new ButtonBuilder().setStyle(ButtonStyle.Primary).setCustomId(`controller:${guild}:Replay`).setEmoji("⏮️"), new ButtonBuilder().setStyle(player.playing ? ButtonStyle.Primary : ButtonStyle.Danger).setCustomId(`controller:${guild}:PlayAndPause`).setEmoji(player.playing ? "⏸️" : "▶️"), new ButtonBuilder().setStyle(ButtonStyle.Primary).setCustomId(`controller:${guild}:Next`).setEmoji("⏭️"), new ButtonBuilder().setStyle(player.repeatMode !== "off" ? ButtonStyle.Success : ButtonStyle.Danger).setCustomId(`controller:${guild}:Loop`).setEmoji(player.repeatMode === "track" ? "🔂" : "🔁"));
  }
}
