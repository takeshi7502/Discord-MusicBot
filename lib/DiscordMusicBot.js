    ...
    this.manager
      ...
      .on("queueEnd", async (player, track) => {
        ...
        if (!autoQueue || !tracksAdded) {
          ...
          // Sửa tin nhắn "Đang phát" thành "Hàng đợi đã kết thúc" thay vì xoá rồi gửi mới
          const existingMsg = player.get("nowPlayingMessage");
          let statusMsg = null;
          if (existingMsg && !client.isMessageDeleted(existingMsg)) {
            try {
              ...
              // Nếu edit thất bại thì gửi message mới
          ...
          // Đánh dấu message cũ đã không còn là now playing
            ...
            if (!player.playing && !twentyFourSeven) {
              setTimeout(async () => {
                if (!player.playing && player.connected) {
                  ...
                  // Sửa tin nhắn "Hàng đợi đã kết thúc" thành "Đã ngắt kết nối!"
                  const currentMsg = player.get("nowPlayingMessage");
                  if (currentMsg && !client.isMessageDeleted(currentMsg)) {
                    try {
                      await currentMsg.edit({ embeds: [disconnectedEmbed], components: [] });
                    } catch {
                      const ch = client.channels.cache.get(player.textChannelId);
                      if (ch) await ch.send({ embeds: [disconnectedEmbed] }).catch(() => {});
                    }
                  } else {
                    const ch = client.channels.cache.get(player.textChannelId);
                    if (ch) await ch.send({ embeds: [disconnectedEmbed] }).catch(() => {});
