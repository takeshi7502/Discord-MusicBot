...
# Bỏ qua docker khỏi danh sách ép buộc cài bằng apt để tránh xung đột
for pkg in jq ufw curl wget java; do
    # java thì tên package apt là default-jre
    apt_pkg=$pkg
    if [ "$pkg" == "java" ]; then apt_pkg="default-jre"; fi
    
    if ! command -v $pkg &> /dev/null; then
        echo -e "${RED}Chưa cài đặt $pkg. Đang tiến hành cài $apt_pkg...${NC}"
        apt-get update -y
        apt-get install -y $apt_pkg || die "Cài đặt gói $apt_pkg thất bại"
    fi
done

# Kiểm tra Docker và Docker Compose V2 riêng biệt
if ! command -v docker &> /dev/null; then
    die "Chưa cài đặt Docker! Vui lòng cài đặt Docker (https://docs.docker.com/engine/install/ubuntu/) rồi chạy lại script."
fi
if ! docker compose version &> /dev/null && ! command -v docker-compose &> /dev/null; then
    die "Chưa cài đặt tiện ích Docker Compose! Vui lòng cài đặt rồi chạy lại script."
fi

echo -e "\n${YELLOW}[4] Cấu hình file application.yml...${NC}"
cp example.vps.application.yml application.yml || die "Copy file cấu hình thất bại"

# Đổi thông IP, pass, port
...
echo -e "\n${YELLOW}=== 🎵 THÔNG TIN KẾT NỐI NODE (COPY VÀO BOT) 🎵 ===${NC}"
echo -e "📌 Tên Node:  ${GREEN}Lavalink-Vultr (Hoặc tùy bạn đặt)${NC}"
echo -e "📌 Host / IP: ${GREEN}$PUBLIC_IP${NC}"
echo -e "📌 Port:      ${GREEN}$LAVA_PORT${NC}"
echo -e "📌 Mật khẩu:  ${GREEN}$LAVA_PASS${NC}"
echo -e "📌 Secure:    ${GREEN}false${NC} (Không dùng SSL)"
echo -e "${GREEN}===================================================${NC}"
